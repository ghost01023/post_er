...
//                                     //checks if feed has been modified before
//                                     if (!feed_data_json["firstAccess"]) {
//                                         data_json.map(user => {
//                                             feed_data_json["userPostDictionary"][user] = {
//                                                 fetched: 0, //SET IT TO DIFF AMOUNT
//                                                 endIn: 3, //SET IT TO DIFF RANGE (BOTH BASED ON USER FEED JSON DATA INPUT). BUT LATER
//                                                 endOfPosts: false
//                                             }
//                                         })
//                                         console.log("json file feed_data after mapping through followed(s) is ")
//                                         console.log(feed_data_json)
//                                         resolve(data_json)
//
//                                     }
...
//                             // fs.appendFile((usersPath + email_username + "/feed/feed.json"), )
//                             // data_json.map(user => {
//                             //
//                             // })
//                         }
...
//         console.log("Second")
//         //SET USER DICTIONARY FETCHED AND END_IN VALUES AND THEN APPEND FEED VALUES AND IN EACH SUCCESSIVE FETCH
//         following.map(user => {
...
//             }
//         })
//         //REPLACE THE mapped FOLLOWING WITH A FEED_OBJECT FROM feed.json AND HAVE IT BE A JSON PARSED OBJECT[1]/[0]
//         //ETC SO THAT THE OTHER DATA MEMBER REPRESENTS CURRENT FEED STATE - i.e., (EITHER AN EMPTY ARRAY OR ALL
//         // THE POSTS OF 20 OR ~20 OF ALL FOLLOWED USERS)
...
//     if (dataJson.length > fetchCount && dataJson.length >= limit) {
//         for (let i = fetchCount; i < limit; i++) {
//             postArray = postArray.concat(dataJson[i]);
//             fetchCount++
//         }
//     } else if (dataJson.length < fetch) {
