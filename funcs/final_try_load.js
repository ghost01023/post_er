            ...
            return new Promise((resolve, reject) => {
                if (feedObject["firstAccess"]) {
                    following.map(async user => {
                        ...
                        await FetchUserPosts(email_username, user, fetched, endIn).then((postArray) => {
                            ...
                            if (traversed === traversal) {
                                ...
                                fs.readFile((usersPath + email_username + "/feed/feed.json"), (err, data) => {
                                    let dataJson = JSON.parse(data.toString())
                                    console.log("Here's what we will write: ")
                                    console.log(stablePostArray)
                                    console.log("Here's what we will write it to: ")
    ...
    return new Promise((resolve, reject) => {
        fs.readFile((usersPath + username + "/posts/posts.json"), (err, data) => {
            if (err) {
                ...
                singleUserPostDict["fetched"] = fetchCount;
                singleUserPostDict["endOfPosts"] = endOfPosts;
